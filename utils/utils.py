...
def farthest_points(data, nclusters, dist_func, return_center_indexes=False, return_distances=False, verbose=False):
    """
      Performs farthest point sampling on data points. FPS algorithm
      Args:
      ...
      Returns clusters, [centers, distances]:
        clusters: numpy array containing the cluster index for each element in data.
        centers: numpy array containing the integer index of each center.
        distances: numpy array of [npoints] that contains the closest distance of each point to any of the cluster centers.
    """
...
def distance_by_translation_grasp(p1, p2):
    """Gets two nx4x4 numpy arrays and computes the translation of all the grasps."""
    t1 = p1[:, :3, 3]
    t2 = p2[:, :3, 3]
...
def evaluate_grasps(grasp_tfs, obj_mesh):
    """
    Check the collision of the grasps and also heuristic quality for each grasp.
    """
    obj_mesh = obj_mesh.mesh
    collisions, _ = sample.in_collision_with_gripper(obj_mesh, grasp_tfs, gripper_name='panda', silent=True)
    qualities = sample.grasp_quality_point_contacts(grasp_tfs, collisions, object_mesh=obj_mesh, gripper_name='panda', silent=True)
    return np.asarray(collisions), np.asarray(qualities)


def merge_pc_and_gripper_pc(pc, gripper_pc, instance_mode=0, pc_latent=None, gripper_pc_latent=None):
    """
    Merges the object point cloud and gripper point cloud and adds a binary auxiliary feature
    that indicates whether each point belongs to the object or to the gripper.
    """
    pc_shape = pc.shape
...
def get_equidistant_points(p1, p2, parts=10):
    """Return linear interpolation points(#parts+1: include endpoint)"""
...
def get_control_point_tensor(batch_size, use_torch=True, dense=False, device="cpu"):
    """
    Output a tensor of shape (batch_size x 6 x 3).
    use_torch: Switch between tensor type and numpy array type
    dense: If True, it will create a dense grid of control points.
    """
    # Load the control points from a .npy file
    panda_npy_path = os.path.join(ROOT_PATH, 'gripper_control_points/panda.npy')
    control_points = np.load(panda_npy_path)[:, :3]  # Only take the first three columns (x, y, z coordinates)

    if dense:
        # If dense is True, create a dense grid of control points
        # Generate points between the two main control points
        points_between_fingers = get_equidistant_points(control_points[0, :], control_points[1, :])
        # Generate points from the base to the middle of the two main control points
        points_from_base_to_gripper = get_equidistant_points([0, 0, 0], (control_points[0, :] + control_points[1, :]) / 2.0)
        # Concatenate all the control points including the origin twice, the points between fingers,
        # the points from base to gripper, and the original control points
        control_points = np.concatenate(([[0, 0, 0], [0, 0, 0]], points_between_fingers, points_from_base_to_gripper, control_points))
    else:
        # If dense is False, use a predefined set of control points
        # This includes the origin twice, the first two control points from the loaded data,
        # and the last two control points from the loaded data
        control_points = [[0, 0, 0], [0, 0, 0],
                          control_points[0, :], control_points[1, :],
                          control_points[-2, :], control_points[-1, :]]

    # Convert the control points to a numpy array of type float32
    control_points = np.asarray(control_points, dtype=np.float32)
    # Expand dimensions to add a batch dimension and repeat the control points for each item in the batch
    control_points = np.tile(np.expand_dims(control_points, 0), [batch_size, 1, 1])

    if use_torch:
        # If use_torch is True, convert the numpy array to a PyTorch tensor and move it to the specified device
        return torch.tensor(control_points).to(device)
    
    # Return the control points as a numpy array
...
def transform_control_points(gt_grasps, batch_size, mode='qt', device="cpu", dense=False):
    """
      ...
      mode = 'qt' expects gt_grasps to have (batch_size x 7)
             where each element is concatenation of quaternion and translation for each grasp.
      mode = 'rt': expects gt_grasps to have shape (batch_size x 4 x 4)
             where each element is 4x4 transformation matrix of each grasp.
    """
    assert (mode == 'qt' or mode == 'rt'), mode
    
    # Get the shape of the gt_grasps tensor
    grasp_shape = gt_grasps.shape
    
    if mode == 'qt':
        # For 'qt' mode, ensure the shape of gt_grasps is (batch_size, 7)
        assert (len(grasp_shape) == 2), grasp_shape
        assert (grasp_shape[-1] == 7), grasp_shape
        
        # Generate control points tensor
        control_points = get_control_point_tensor(batch_size, device=device, dense=dense) # (batch_size, 6, 3)
        num_control_points = control_points.shape[1]
        
        input_gt_grasps = gt_grasps
        # Add a new dimension to gt_grasps and repeat it for each control point
        gt_grasps = torch.unsqueeze(input_gt_grasps, 1).repeat(1, num_control_points, 1) # (batch_size, 6, 7)
        
        # Separate quaternion (first 4 values) and translation (last 3 values)
        gt_q = gt_grasps[:, :, :4]
        gt_t = gt_grasps[:, :, 4:]
        
        # Rotate control points using the quaternion
        gt_control_points = qrot(gt_q, control_points)
        
        # Add translation to the control points
        gt_control_points += gt_t
        
        return gt_control_points
    
    else:  # 'rt' mode
        # For 'rt' mode, ensure the shape of gt_grasps is (batch_size, 4, 4)
        ...
        # Add homogeneous coordinates to the control points
        ones = torch.ones((shape[0], shape[1], 1), dtype=torch.float32)
        control_points = torch.cat((control_points, ones), -1)
        
        # Transform control points using the transformation matrix
...
def transform_control_points_numpy(gt_grasps, batch_size, mode='qt', dense=False):
    ...
    if mode == 'qt':
        ...
        num_control_points = control_points.shape[1]
        input_gt_grasps = gt_grasps
        gt_grasps = np.expand_dims(input_gt_grasps, 1).repeat(num_control_points, axis=1)
        gt_q = gt_grasps[:, :, :4]
        gt_t = gt_grasps[:, :, 4:]
...
def rotate_point_by_quaternion(point, q, device="cpu"):
    """
    Takes in points with shape of (batch_size x n x 3) and quaternions with shape of (batch_size x n x 4)
    and returns a tensor with shape of (batch_size x n x 3)
    which is the rotation of the point with quaternion q.
    """
    shape = point.shape
    q_shape = q.shape
    ...
    assert (q_shape[1] == shape[1]), 'point shape = {} q shape = {}'.format(shape, q_shape)

    q_conj = conj_quaternion(q)

    r = torch.cat([torch.zeros((shape[0], shape[1], 1), dtype=point.dtype).to(device), point], dim=-1)
    final_point = quaternion_mult(quaternion_mult(q, r), q_conj)
    final_output = final_point[:, :, 1:]  # torch.slice(final_point, [0, 0, 1], shape)
    return final_output
...
def get_inlier_grasp_indices_with_control_points(grasps, query_point=torch.tensor([[0.0, 0.0, 0.0]]), threshold=0.4, device="cpu"):
    """Return all grasps whose distance between the mid of the fingertips and the query point is less than the threshold value.
    Arguments:
        grasps are given as a tensor of [B,6,3]
        where B is the number of grasps,
        6 is the number of control points, and
        3 is their position in Euclidean space.
