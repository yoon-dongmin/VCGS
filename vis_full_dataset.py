...
def get_pickle_data_path():
    data_path = os.path.join(ROOT_PATH, "dataset/full_dataset")
...
def preprocess_data(data):
    ...
    print("---" * 30)
    print("[0] element of Is: LEN {}, TYPE {}".format(len(query_points_with_grasps_for_current_point_cloud[0]),
                                                      type(query_points_with_grasps_for_current_point_cloud[0])))
    print("Mean number of points in each cluster: ",
          np.mean([len(x) for x in query_points_with_grasps_for_current_point_cloud]))
    print("---" * 30)
    print("[0][0] element of Is: LEN {}, TYPE {}".format(query_points_with_grasps_for_current_point_cloud[0][0].shape,
                                                         type(query_points_with_grasps_for_current_point_cloud[0][0])))
    print("Mean number of points in each cluster: ",
    ...
    print("---" * 30)
    print("[0] element of Gs: LEN {}, TYPE {}".format(
        len(grasp_indices_for_each_point_with_grasp_on_each_rendered_point_cloud[0]),
        type(grasp_indices_for_each_point_with_grasp_on_each_rendered_point_cloud[0])))
    print("Mean number of grasps in each cluster: ",
          np.mean([len(x) for x in grasp_indices_for_each_point_with_grasp_on_each_rendered_point_cloud]))
    print("---" * 30)
    print("[0][0] element of Gs: LEN {}, TYPE {}".format(
        grasp_indices_for_each_point_with_grasp_on_each_rendered_point_cloud[0][0].shape,
        type(grasp_indices_for_each_point_with_grasp_on_each_rendered_point_cloud[0][0])))
    print("Mean number of grasps in each cluster: ",
          np.mean([x[0].shape[0] for x in grasp_indices_for_each_point_with_grasp_on_each_rendered_point_cloud]))

    # Load 7896 files
    # constrained_TV_c7ab551c69d449e17f42f6c9f9e08b68_0_00341218973694903.pickle
    # ==========================================================================================
    # Total number of clusters:  100
    # ------------------------------------------------------------------------------------------
    # [0] element of Is: LEN 50, TYPE <class 'list'>
    # Mean number of points in each cluster:  47.33
    # ------------------------------------------------------------------------------------------
    # [0][0] element of Is: LEN (224,), TYPE <class 'numpy.ndarray'>
    # Mean number of points in each cluster:  355.99
    # ==========================================================================================
    # Total number of grasps:  100
    # ------------------------------------------------------------------------------------------
    # [0] element of Gs: LEN 50, TYPE <class 'list'>
    # Mean number of grasps in each cluster:  47.33
    # ------------------------------------------------------------------------------------------
    # [0][0] element of Gs: LEN (182,), TYPE <class 'numpy.ndarray'>
    # Mean number of grasps in each cluster:  275.75
