...
class BaseOptions:
    ...
    def initialize(self):
        # data params
        self.parser.add_argument('--arch', choices={"sampler", "evaluator"}, default="sampler")
        self.parser.add_argument('--dataset_root_folder', '-d', type=str, default='./dataset/generated_dataset/grasps', help='path to root directory of the dataset.')
        ...
        self.parser.add_argument('--pointnet_radius', type=float, default=0.02, help='Radius for ball query for PointNet++, just the first layer')
        self.parser.add_argument('--pointnet_nclusters', type=int, default=128, help='Number of cluster centroids for PointNet++, just the first layer')
        ...
        self.parser.add_argument('--dataset_split_ratio', '-dsr', type=float, nargs=3, default=[0.8, 0.15, 0.05], help='This splits the training dataset into a train, test, and validation set according to the ratios specified.')
        self.parser.add_argument('--extra_name', '-en', type=str, default="", help='Extra name to append to folder name.')
        self.parser.add_argument('--no_vis', action='store_true', help='if true, no visualizations are stored')
        self.parser.add_argument('--caching',  '-c', action='store_true', help='If true, then once training data is read from a file it is cached in a large list. Warning, this can consume a lot of RAM memory!')
    ...
    def parse(self):
        ...
        if self.opt.is_train:
            ...
            name += "_scale_" + str(self.opt.model_scale) + "_npoints_" + str(self.opt.pointnet_nclusters)
            name += "_radius_" + str(self.opt.pointnet_radius).split(".")[-1]
