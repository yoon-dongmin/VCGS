...
class ConstrainedGraspSamplingData(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            (point_clouds,
             clusters_with_grasps_per_point_cloud,
             grasps_per_clusters,
             prerendered_camera_poses,
             mesh_file,
             mesh_scale) = self.read_grasp_file(path)

            NUM_GRASPS_PER_OBJECT = 1 if self.validate else self.opt.num_grasps_per_object
            (sampled_point_cloud_indices,
             sampled_query_point_indices_with_grasps_per_point_cloud,
             sampled_grasps_per_query_point) = self.sample_clusters_query_points_and_grasps(NUM_GRASPS_PER_OBJECT,
                                                                                            point_clouds,
                                                                                            clusters_with_grasps_per_point_cloud,
                                                                                            grasps_per_clusters)
        ...
        gt_control_points = utils.transform_control_points_numpy(sampled_grasps_per_query_point, NUM_GRASPS_PER_OBJECT, mode='rt')[:, :, :3]

        pc = point_clouds[sampled_point_cloud_indices]
        features = self.create_constrained_features(pc.shape[0], pc.shape[1], sampled_query_point_indices_with_grasps_per_point_cloud)
        ...
        if self.validate:
            ...
            meta['mesh_file'] = np.array([mesh_file] * self.opt.num_grasps_per_object)
            meta['mesh_scale'] = np.array([mesh_scale] * self.opt.num_grasps_per_object)
            meta['camera_pose'] = prerendered_camera_poses[sampled_point_cloud_indices]

        meta['pc'] = pc
        meta['features'] = features
        meta['grasp_rt'] = sampled_grasps_per_query_point.reshape(len(sampled_grasps_per_query_point), -1)
    ...
    def sample_clusters_query_points_and_grasps(self, num_samples, point_clouds, clusters_with_grasps, grasps_per_cluster):
        ...
        for point_cloud_index in point_cloud_indices:
            ...
            random_cluster_indices_with_grasps.append(clusters_with_grasps[point_cloud_index][random_cluster_index_with_grasp])
            ...
            random_grasps_per_cluster.append(grasps_per_cluster[point_cloud_index][random_cluster_index_with_grasp][random_grasp_index_for_random_query_point])
    ...
    def read_grasp_file(self, path):
        ...
        if self.caching and file_name in self.cache:
            point_clouds, query_points_with_grasps, grasps_per_query_points, prerendered_camera_poses, mesh_file, mesh_scale = copy.deepcopy(self.cache[file_name])
            return point_clouds, query_points_with_grasps, grasps_per_query_points, prerendered_camera_poses, mesh_file, mesh_scale

        point_clouds, query_points_with_grasps, grasps_per_query_points, prerendered_camera_poses, mesh_file, mesh_scale = self.read_object_grasp_data(path)

        if self.caching:
            self.cache[file_name] = (point_clouds, query_points_with_grasps, grasps_per_query_points, prerendered_camera_poses, mesh_file, mesh_scale)
    ...
    def read_object_grasp_data(self, file_path):
        """
        Reads the grasps from the json path and loads the mesh and all the grasps.
        """
        # print(file_path)
        # print("sampler")
        # exit()
        (grasps,
         pre_rendered_point_clouds,
         camera_poses_for_prerendered_point_clouds,
         all_query_points_per_point_cloud,
         grasp_indices_for_every_query_point_on_each_rendered_point_cloud,
         mesh_file,
         mesh_scale) = self.load_data_from_file(file_path)

        (clusters_with_grasps_for_each_point_cloud,
         grasps_for_each_cluster_per_point_cloud,
         point_clouds_to_keep) = self.get_query_points_with_grasps_and_all_grasps(all_query_points_per_point_cloud,
                                                                                  grasp_indices_for_every_query_point_on_each_rendered_point_cloud,
    ...
    def load_from_h5py_file(self, h5py_file):
        ...
        grasp_indices_for_every_query_point_on_each_rendered_point_cloud = h5_dict["query_points/grasp_indices_for_each_point_with_grasp_on_each_rendered_point_cloud"][()]
...
if __name__ == "__main__":
    ...
    training_dataset_loader = dataloader_.create_dataloader(training_dataset, shuffle_batches=not opt.serial_batches)
    ...
    for i, data in enumerate(training_dataset_loader):
        ...
        print(f"Target Center Points: {data['target_cps'].shape}")
